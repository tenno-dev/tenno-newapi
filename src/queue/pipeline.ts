...
export async function handlePrepareWorldStateRun(
  ...
  const analysis = await analyzeWorldStateDiffs(env.kv, worldState, message.force);
  ...
  for (const processMessage of processMessages) {
    await env.queue.send(processMessage);
  }
}
...
export async function handleProcessWorldStateRoot(
  ...
  const previousValue = await loadCurrentRootPayload(env.kv, message.rootKey);
  ...
  if (hasRootKey) {
    await Promise.all([
      ...
      saveLastKnownRootPayload(env.kv, message.rootKey, payload),
    ]);
    ...
    for (const translateMessage of translateMessages) {
      await env.queue.send(translateMessage);
    }
  } else {
