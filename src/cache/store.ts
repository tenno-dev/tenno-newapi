...
async function loadCursor(kv: KVStore, cursorKey: string): Promise<CurrentWriteCursor | null> {
...
export async function loadRootHashes(kv: KVStore): Promise<RootHashMap> {
...
export async function loadRawSnapshot(kv: KVStore, runId: string): Promise<string | null> {
  return kv.get(buildRawSnapshotKey(runId));
}

export async function loadRawSnapshotByKey(kv: KVStore, key: string): Promise<string | null> {
  return kv.get(key);
}
...
export async function deleteCurrentRootPayload(kv: KVStore, rootKey: string): Promise<void> {
...
export async function loadLatestRunMeta(kv: KVStore): Promise<LatestRunMeta | null> {
