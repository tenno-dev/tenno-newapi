...
async function updateRunExecutionState(db: SQLClient, runId: string): Promise<void> {
...
export async function persistWorldStateRun(
  ...
  const rawSnapshotKey = await saveRawSnapshot(env.kv, input.runId, input.rawPayload);
  const changedPayloadKeys: Array<{ rootKey: string; kvKey: string }> = [];

  for (const item of input.changedPayloadValues) {
    const kvKey = await saveRootPayload(env.kv, item.rootKey, input.runId, item.payload);
    changedPayloadKeys.push({ rootKey: item.rootKey, kvKey });
  }

  await saveLatestRunMeta(env.kv, {
    runId: input.runId,
  ...
  for (const item of input.changed) {
    await env.sql
      .prepare(SQL.insertPipelineDiff)
  ...
  if (input.itemChanges.length > 0) {
    await env.sql.batch(
      input.itemChanges.map((item) =>
        env.sql.prepare(SQL.insertPipelineItemChange).bind(
          input.runId,
          item.rootKey,
          item.itemId,
          item.changeType,
...
export async function recordPreparedWorldStateRun(
  ...
  await saveRunSummary(env.kv, input.runId, {
    runId: input.runId,
    fetchedAt: input.fetchedAt,
    sourceVersion: input.sourceVersion,
...
export async function writeRootChange(
  ...
  const kvKey = await saveRootPayload(env.kv, input.rootKey, input.runId, input.payload);
...
export async function finalizeWorldStateRun(
  ...
  await saveRunSummary(env.kv, input.runId, summary);
...
export async function getPipelineRunCount(db: SQLClient): Promise<number> {
  await ensureDiffTables(db);
  const runCountQuery = await db.prepare(SQL.countPipelineRuns).bind().all<{ count: number }>();
...
export async function writeDummyTranslationArtifact(
  ...
  await env.kv.put(dummyKey, JSON.stringify(dummyResult), {
...
export async function logQueueProcessed(
  ...
  try {
    ...
    await db
      .prepare(SQL.updateTranslateQueueLogByRunAndRoot)
      .bind(
        input.payloadKey,
        JSON.stringify(input.targetLanguages),
        input.payloadSize,
