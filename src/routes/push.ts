...
async function checkRateLimit(c: Context<AppEnv>, bucket: string): Promise<boolean> {
  ...
  const currentRaw = await c.env.TENNODEV_WORLDSTATE_KV.get(key);
  const current = currentRaw ? Number(currentRaw) : 0;
  if (Number.isFinite(current) && current >= limit) {
    return false;
  }

  await c.env.TENNODEV_WORLDSTATE_KV.put(key, String((Number.isFinite(current) ? current : 0) + 1), {
...
pushRoutes.post("/push/subscribe", async (c) => {
  ...
  if (normalizedRootKeys.includes("*")) {
    await c.env.TENNODEV_WORLDSTATE_D1.batch([
      c.env.TENNODEV_WORLDSTATE_D1.prepare(SQL.deletePushSubscriptionRootKeysBySubscriptionId).bind(id),
      c.env.TENNODEV_WORLDSTATE_D1.prepare(SQL.deletePushSubscriptionSubKeysBySubscriptionId).bind(id),
      c.env.TENNODEV_WORLDSTATE_D1.prepare(SQL.insertPushSubscriptionRootKey).bind(id, "*", now),
    ]);
  } else {
    const statements = [
      c.env.TENNODEV_WORLDSTATE_D1.prepare(SQL.deletePushSubscriptionRootKeysBySubscriptionId).bind(id),
      ...
      ...normalizedRootKeys.map((rootKey) =>
        c.env.TENNODEV_WORLDSTATE_D1.prepare(SQL.insertPushSubscriptionRootKey).bind(id, rootKey, now)
      ),
      ...Object.entries(subKeyFilters).flatMap(([rootKey, subKeys]) =>
        subKeys.map((subKey) =>
          c.env.TENNODEV_WORLDSTATE_D1.prepare(SQL.insertPushSubscriptionSubKey).bind(id, rootKey, subKey, now)
        )
      ),
    ];
    await c.env.TENNODEV_WORLDSTATE_D1.batch(statements);
  }
...
pushRoutes.post("/push/unsubscribe", async (c) => {
  ...
  await ensurePushTables(c.env.TENNODEV_WORLDSTATE_D1);
  await c.env.TENNODEV_WORLDSTATE_D1.batch([
    c.env.TENNODEV_WORLDSTATE_D1.prepare(SQL.deletePushSubscriptionSubKeysByEndpoint).bind(endpoint),
    c.env.TENNODEV_WORLDSTATE_D1.prepare(SQL.deletePushSubscriptionRootKeysByEndpoint).bind(endpoint),
    c.env.TENNODEV_WORLDSTATE_D1.prepare(SQL.deletePushSubscriptionByEndpoint).bind(endpoint),
  ]);
